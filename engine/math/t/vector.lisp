...
(deftest length-test
  (testing "2d vector length"
	   (ok (= (length (make-float2 1 2))
		  (sqrt 5))))

  (testing "3d vector length"
	   (ok (= (length (make-float3 1 2 3))
		  (sqrt 14))))

  (testing "4d vector length"
	   (ok (= (length (make-float4 1 2 3 4))
		  (sqrt 30)))))
