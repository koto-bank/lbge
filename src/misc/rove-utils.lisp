...
(defun report-results ()
  (let ((failed (rove/core/stats:stats-failed-tests rove/stats:*stats*))
        (passed (rove/core/stats:stats-passed-tests rove/stats:*stats*)))

    (when (> (length passed) 0)
      (format t "~%-------------~%Passed tests:~%")
      (loop :for test :across (rove/core/stats:stats-passed-tests rove/stats:*stats*) :do
