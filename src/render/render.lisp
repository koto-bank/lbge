...
:gl :vk :mtl :dx12" :initarg :backend)
   ...
   (viewport-size
    :documentation "Size of the viewport. A pair, e.g. (800 . 600)"
    :accessor viewport-size
    :initform '(800 . 600))
...
(defun set-current-camera (renderer camera &optional (resize t))
  ...
  (when resize
    (let ((size (viewport-size renderer)))
      (resize-viewport renderer (car size) (cdr size)))))
...
(defun resize-viewport (renderer width height)
  (with-slots (viewport-size) renderer
    (setf viewport-size (cons width height))
    (b:resize-viewport (renderer-backend renderer) renderer
                       width height)))
