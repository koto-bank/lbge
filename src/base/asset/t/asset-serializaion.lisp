...
(deftest asset-serialize-test
  (let* ((mgr (make-instance 'asset-manager))
    ...
    (testing "[De]serialization of assets"
      ...
      (add-handler mgr (make-instance 'small-asset-asset-handler))
      (add-handler mgr (make-instance 'big-asset-asset-handler))
      (let* ((small-from-disk (get-asset mgr small-2-key))
        ...
        (ok (eq :void (slot-value big-small-1 'state)))
        (ok (eq :void (slot-value big-small-2 'state)))
        ...
        (ok (eq :loaded (slot-value big-small-1 'state)))
        (ok (eq :void (slot-value big-small-2 'state)))))))
